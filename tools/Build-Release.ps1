...
@{ sourceCommit = $Revision; configuration = 'Release'; runtime = 'win-x64'; selfContained = $true; selfTests = 'passed'; format = 'passed'; normalTrayStarted = $false; interactiveAcceptance = 'pending' } | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $workRoot 'build-result.json') -Encoding UTF8
